...
def V_grid_move_scale(gbs, ogbs, grs, ogrs, gNei, ogNei, agNei, agInd, aogNei, aogInd, A_inv_current, V_current, V_grid_current, Vmmu1_current, V_gridmmu1_current, A_invVmmu1_current, A_invV_gridmmu1_current, mu_current):
    ...
    for ic in range(n_grid+1):
        for jc in range(n_grid+1):
            ...
            gamma = [np.sum([(A_invV_gridmmu1_current[j,agNei[jc*(n_grid+1)+ic][ni]] - np.inner(A_invV_gridmmu1_current[j,gNei[agNei[jc*(n_grid+1)+ic][ni]]],gbs[j,kay1c(agNei[jc*(n_grid+1)+ic][ni],m,n_grid)]) + A_invV_gridmmu1_current[j,gNei[agNei[jc*(n_grid+1)+ic][ni]]][agInd[jc*(n_grid+1)+ic][ni]]*gbs[j,kay1c(agNei[jc*(n_grid+1)+ic][ni],m,n_grid)][agInd[jc*(n_grid+1)+ic][ni]])/grs[j,kay1c(agNei[jc*(n_grid+1)+ic][ni], m, n_grid)]*gbs[j,kay1c(agNei[jc*(n_grid+1)+ic][ni], m, n_grid)][agInd[jc*(n_grid+1)+ic][ni]] for ni in range(len(agNei[jc*(n_grid+1)+ic]))]) for j in range(p)]
